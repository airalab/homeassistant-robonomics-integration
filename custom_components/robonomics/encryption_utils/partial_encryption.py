...
async def partial_encrypt(
    ...
    for i in range(pieces_count):
        ...
        encrypted_piece = sender_keypair.encrypt_message(
            data_piece, recipient_public_key
        )
        _LOGGER.debug(
            f"{round(i*100/pieces_count)} - Piece size: {len(data_piece)}, encrypted piece size: {len(encrypted_piece.hex())}"
        )
        await hass.async_add_executor_job(
            _write_data_to_file, encrypted_piece.hex(), filename
        )
...
async def partial_decrypt(
    encrypted_message: tp.Union[bytes, str],
    ...
    for i in range(pieces_count):
        ...
        decrypted_piece = receiver_keypair.decrypt_message(
