...
from .sr25519_const import DERIVATION_KEY_SALT_SIZE, NONCE_SIZE
...
def sr25519_encrypt(
    ...
    if sender_keypair is not None:
        ...
        message_keypair = Keypair.create_from_mnemonic(
            mnemonic=Keypair.generate_mnemonic(), crypto_type=KeypairType.SR25519
        )

    # 2. Key agreement
    agreement_key = get_sr25519_agreement(
        secret_key=message_keypair.private_key, public_key=receiver_public_key
    )
    ...
    return bytes_concat(
        nonce, salt, message_keypair.public_key, mac_value, encrypted_message
    )
